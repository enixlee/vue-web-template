...
export const RpcSDKManagePaymentNetPayAddMethod = '/sdkmanage/payment/net_pay/add';

export const RpcSDKManagePaymentNetPayAddRpcType = 'system';

/**
 *
 * @param merchantNo merchantNo 商户类型
 * @param netPayBankOnlineId string 网银在线id
 * @param effectiveTime datetime 生效时间
 * @param endTime datetime 结束时间
 * @returns {*}
 * @constructor
 */
export function RpcSDKManagePaymentNetPayAdd (
  merchantNo,
  netPayBankOnlineId,
  effectiveTime,
  endTime) {
  tc.typeCheckString(netPayBankOnlineId, 1, 16, false);
  ...
  if (debug.isProduction()) {
    return Rpc.post(RpcSDKManagePaymentNetPayAddMethod, params);
  } else {
    return Rpc.get(RpcSDKManagePaymentNetPayAddMethod, params);
  }
}
