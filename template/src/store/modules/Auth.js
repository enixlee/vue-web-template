...
const Auth = {
  ...
  actions: {
    ...
    [Subjects.MEMBER_LOGIN]: ({state, commit, rootState}, payload) => {
      ...
      Vue.prototype.$rpc.RpcSDKZhongBaiAuthLoginWithVerifyCode(
        userName,
        password,
        verifyCode
      )
    ...
    [Subjects.MEMBER_LOGOUT]: ({state, commit, rootState}, payload) => {
      ...
      Vue.prototype.$rpc.RpcSDKZhongBaiAuthLogout();
      commit('CLEAR_STATE_OF_Auth');
    },
    [Subjects.GET_LOGIN_VERIFY_CODE]: ({state, commit, rootState}, payload) => {
      Vue.prototype.$rpc.RpcSDKZhongBaiAuthGetLoginVerifyCode()
        .then((rpcResult) => {
          if (!rpcResult.isSucc()) {
            return;
          }
  ...
  mutations: {
    ...
    [Subjects.MEMBER_LOGIN] (state, payload) {
      ...
      state.expireTime = parseInt(payload.expireTime);
